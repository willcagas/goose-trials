-- Fix: Update hook_validate_university_email to support subdomains
-- This applies the fix from migration 0006_fix_auth_hook_subdomain.sql
-- Run this in Supabase SQL Editor

-- Update Auth Hook function to support subdomain matching
...
BEGIN
  ...
  -- Step 1: Try exact match first (fastest, uses index)
...
BEGIN
  ...
  IF v_result = '{}'::jsonb THEN
    RAISE NOTICE 'SUCCESS: Hook function now supports subdomains!';
  ELSE
    RAISE WARNING 'FAILED: Hook returned: %', v_result;
  END IF;
END $$;
